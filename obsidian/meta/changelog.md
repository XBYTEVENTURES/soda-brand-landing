...
- **Standalone `index.html` added at the repo root — deliberately outside the
  hard rules.** A single-file recreation of an external "Diet Soda" hero landing
  page (3D can via Google `<model-viewer>`, cursor-tilt, pointer-repelled berry
  models, flavor-switch choreography), built to a supplied verbatim spec. It is
  **not part of the Next.js app** — no route, no view, no React, not imported by
  anything, and it does not run through `next build`.
  **It breaks hard rules #1, #4, #5 and #6 on purpose:** motion is GSAP +
  `@keyframes` rather than `@react-spring/web`; colours, sizes and durations are
  hardcoded hex/px instead of design tokens; there is no route→view delegation
  and no Server Component boundary. The deliverable was specified as one
  self-contained file, no build step, CDN `<script>` tags only — the spring
  engine, the three-tier token convention and the view layer have no meaning
  outside the app. Those rules govern `src/`; this is a sibling artifact.
  **Do not "port it to springs" or tokenise it** unless explicitly asked — that
  would destroy the thing it was built to be.
  No dependencies were added (`package.json` untouched) and nothing was written
  to `public/` — all nine assets (GLB models, can textures, bubble PNG) load from
  a remote CDN. Verified: every asset URL returns 200, GSAP and the
  `model-viewer` custom element both register, element counts and the default
  teal gradient resolve correctly. **Motion is unverified** — the preview pane
  does not composite, so `requestAnimationFrame` fires zero frames and both
  GSAP's ticker and `model-viewer`'s loader stay parked; open the file in a real
  browser to confirm the spin, parallax and texture swap. Listed in
  [[folder-structure]]. No ADR — no architectural choice was made about the app.
