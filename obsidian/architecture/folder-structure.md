...
└── index.html               ← standalone single-file page — NOT part of the app (see below)
```
...
> [!warning] Root `index.html` is not application code
> `index.html` is a self-contained recreation of an external landing page —
> pure HTML/CSS/JS with CDN `<script>` tags (GSAP, `<model-viewer>`), no build
> step. Nothing imports it and `next build` never sees it. It **intentionally
> breaks hard rules #1, #4, #5 and #6** (GSAP + `@keyframes` instead of springs,
> hardcoded hex/px instead of tokens, no route→view delegation, no React) because
> the brief was "one file, no framework". Leave it alone: do not port its motion
> to `@react-spring/web`, do not tokenise its values, and do not treat it as
> precedent for how `src/` may be written. Logged in [[changelog]] (2026-08-04).
